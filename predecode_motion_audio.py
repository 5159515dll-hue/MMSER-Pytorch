...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="If >0, only predecode the first N selected samples after split filtering",
    )
...
def main():
    ...
    if split_manifest_path is not None:
        ...
        manifest_items = select_manifest_items(manifest, str(args.subset))
        if int(args.limit) > 0:
            manifest_items = manifest_items[: int(args.limit)]
        for item in manifest_items:
            label_name = str(item.get("label_en", "") or "").strip()
            if label_name not in EMOTIONS:
                continue
            seq = _normalize_seq(item.get("seq"))
            if not seq:
                continue
            rows.append(
                {
    ...
    source_desc = str(split_manifest_path) if split_manifest_path is not None else str(xlsx)
    print(f"Loaded {len(rows)} labeled rows from {source_desc}", flush=True)
    if split_manifest_path is not None:
        print(f"Using split manifest: {split_manifest_path} (subset={args.subset}, sha256={manifest_hash})", flush=True)
    ...
    for item in rows:
        ...
        video_path = Path(item["video_path"]).expanduser() if item.get("video_path") else None
        audio_path = Path(item["audio_path"]).expanduser() if item.get("audio_path") else None
